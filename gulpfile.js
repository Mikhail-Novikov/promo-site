...
gulp.task('update-dependencies', function(){
	...
	var arcticModal = gulp.src([
        'app/libs/arcticModal/arcticmodal/jquery.arcticmodal.js'
	]).pipe(rename('jquery.arcticmodal-0.3.min.js')).pipe(gulp.dest('app/js'));
	var nanoscroller = gulp.src([
        'app/libs/nanoscroller/bin/javascripts/jquery.nanoscroller.js',
	]).pipe(rename('nanoScroller.js')).pipe(gulp.dest('app/js'));
...
gulp.task('watch', ['browser-sync', 'build'], function() {
	...
	gulp.watch('app/svg/sprite/*', ['sprite']).on('change', function(evt) { changeEvent(evt); });
...
gulp.task('build', ['sass', 'concat','spritesmith','jade','scripts','sprite']);
